...
import {RouterFacet} from "./UniswapV4RouterLibrary.sol";
...
contract UniswapV4Router is ILockCallback {
    ...
    RouterFacet public immutable routeFacet;

    constructor(IPoolManager _manager, RouterFacet _routeFacet) {
        manager = _manager;
        routeFacet = _routeFacet;
    }
    ...
    function lockAcquired(
        ...
        for (uint256 i = 0; i < calls.length; ++i) {
            ...
            if (call.results) {
                ...
                bytes memory dataNoSelector = routeFacet.removeSelector(
                    call.data
                );
                // decode the param and ignore the result
                (bytes memory decodedCallData, ) = abi.decode(
                    dataNoSelector,
                    (bytes, bytes)
                );
            ...
            if (call.callType == CallType.Delegate) {
                ...
                console.log("INfo", call.target, call.value);
